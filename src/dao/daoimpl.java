...
public class daoimpl implements dao {
    ...
    public String checkclassgrade(int taskid, int classid) {
        Statement statement = null;
        String s = "";
        try {
            statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from task"+taskid+"fromclass"+classid+" where paper_state=3");
            while (resultSet.next()) {
                s+=resultSet.getString("student_name");
                s+=" ";
                s+=resultSet.getInt("paper_point");
                s+=" ";
            }
            return s;
        } catch (Exception exception) {
